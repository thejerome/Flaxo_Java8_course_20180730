    ...
    public void testTotalJobDurationPerNameAndSurname(){
        ...
        Map<String, Integer> collected = getEmployees().stream()
                ...
                (map, employee) -> {
                     ...
                     Integer totalJobDuration = employee.getJobHistory()
                             .stream()
                            .mapToInt(JobHistoryEntry::getDuration)
                            .sum();
                    map.put(person.getFirstName(), map.getOrDefault(person.getFirstName(), 0) + totalJobDuration);
                    map.put(person.getLastName(), map.getOrDefault(person.getLastName(), 0) + totalJobDuration);
                },
                HashMap::putAll
        );
