    ...
    public void indexPersonsByEmployer2() {
        ...
        final Map<String, List<Person>> index = employees.stream()
                       .flatMap(employee -> employee.getJobHistory()
                                                    .stream()
                                                    .map(jobHistoryEntry -> new PersonEmployer(
                                                        employee.getPerson(),
        ...
        final Map<Person, Integer> personDuration =
            employees.stream()
                     .collect(Collectors.toMap(Employee::getPerson,
                         job -> job.getJobHistory()
                                   .stream()
                                   .mapToInt(JobHistoryEntry::getDuration)
                                   .sum())); // TODO use sumAllPersonDurations
    ...
    public void getDurationsForEachPersonByPosition() {
        ...
        final List<PersonPositionDuration> personPositionDurations =
            employees.stream()
                     ...
                     .flatMap(
                         personPositionIndex ->
                             personPositionIndex.getDurationByPositionIndex()
                                                .entrySet()
                                                .stream()
                                                .map(
                                                    entry -> new PersonPositionDuration(
                                                        personPositionIndex.getPerson(),
                                                        entry.getKey(),
                                                        entry.getValue())))
                     .collect(Collectors.toList()); // TODO
    ...
    public void getCoolestPersonByPosition1() {
        ...
        final Map<String, PersonPositionDuration> coolestPersonByPosition =
            employees.stream()
                     ...
                     .map(StreamsExercise::getPersonPositionIndex)
                     .flatMap(
                         personPositionIndex -> personPositionIndex.getDurationByPositionIndex()
                                                                   .entrySet()
                                                                   .stream()
