...
public class StreamsExercise {
    ...
    public void getAllJobHistoryEntries() {
        ...
        final List<JobHistoryEntry> jobHistoryEntries = employees.stream()
                .flatMap(employee -> employee.getJobHistory().stream()).collect(Collectors.toList()); // TODO
    ...
    public void getSumDuration() {
        ...
        final int sumDurations = employees.stream()
                ...
                .mapToInt(JobHistoryEntry::getDuration).sum(); // TODO
    ...
    public static <T> Predicate<T> distinctByProperty(Function<T, ?> function) {
        Map<Object, Boolean> hashMap = new HashMap<>();
        return t -> hashMap.putIfAbsent(function.apply(t), true) == null;
    }
    ...
    public void getDurationsForEachPersonByPosition() {
        ...
        final List<PersonPositionDuration> personPositionDurations = employees.stream()
                .flatMap(employee -> employee.getJobHistory().stream()
                        ...
                        .filter(distinctByProperty(personPositionDuration -> personPositionDuration.position)))
    ...
    public void getCoolestPersonByPosition1() {
        ...
        final Map<String, PersonPositionDuration> coolestPersonByPosition = employees.stream().flatMap(employee -> employee.getJobHistory().stream()
                .map(jobHistoryEntry -> new PersonPositionDuration(employee.getPerson(), jobHistoryEntry.getPosition(), jobHistoryEntry.getDuration())))
                .collect(Collectors.groupingBy(PersonPositionDuration::getPosition, Collectors.collectingAndThen(
                        ...
                        personPositionDuration -> personPositionDuration.orElseThrow(RuntimeException::new)
                )));
    ...
    public void getCoolestPersonByPosition2() {
        ...
        final Map<String, Person> coolestPersonByPosition = employees.stream().flatMap(employee -> employee.getJobHistory().stream()
                .map(jobHistoryEntry -> new PersonPositionDuration(employee.getPerson(), jobHistoryEntry.getPosition(), jobHistoryEntry.getDuration())))
                .collect(Collectors.groupingBy(PersonPositionDuration::getPosition, Collectors.collectingAndThen(
                        Collectors.maxBy(Comparator.comparingInt(PersonPositionDuration::getDuration)),
                        personPositionDuration -> personPositionDuration.orElseThrow(RuntimeException::new).getPerson()
                ))); // TODO
